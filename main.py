...
id = '5999349912'
tok = '7119056147:AAFS1LhCeo_4BohQu1_iUk6Lnq-SKm1OEC4'  # Replace This For Your Token
zzk = 0
...
ـ @maho_s9'''
    ...
    but2 = types.InlineKeyboardButton(text='- ⚜️ 𝐃𝐞𝐯', url='https://t.me/Pythonprojects')
    ...
    bot.send_message(message.chat.id,
                     f"<strong>اهلا بك : | {nm} | مرحبا بك في بوت info SocialMedia اختر الخدمه التي تعجبك من الازرار الشفافه ايضا للحصول على معلوماتك اضغط على  [ /info ]</strong>",
...
def st(call):
    if call.data == 'Tik':
        ...
        Az = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                   text='ارسل اسم المستخدم تيك توك', reply_markup=nc1)
        bot.register_next_step_handler(Az, zd2)
    elif call.data == "IG":
        nc1 = types.InlineKeyboardMarkup()
        zd1 = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                    text='ارسل اسم المستخدم انستاجرام', reply_markup=nc1)
        bot.register_next_step_handler(zd1, OZ)
    elif call.data == "Tw":
        nc1 = types.InlineKeyboardMarkup(row_width=2)
        MC = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                   text='ارسل اسم المستخدم تويتر X', reply_markup=nc1)
        bot.register_next_step_handler(MC, k3)
    elif call.data == 'Sn':
        nc1 = types.InlineKeyboardMarkup(row_width=2)
        Bz = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                   text='ارسل اسم المستخدم سناب شات', reply_markup=nc1)
        bot.register_next_step_handler(Bz, PO)
    elif call.data == "YT":
        nc1 = types.InlineKeyboardMarkup()
        tell = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                     text='ارسل اسم المستخدم يوتيوب', reply_markup=nc1)
        bot.register_next_step_handler(tell, VI)
    elif call.data == "Tele":
        nc1 = types.InlineKeyboardMarkup(row_width=2)
        Qr = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                   text='ارسل اسم المستخدم الذي تريد تبحث عليه في التيليجرام', reply_markup=nc1)
        bot.register_next_step_handler(Qr, GH)
    elif call.data == "PH":
        nc1 = types.InlineKeyboardMarkup(row_width=2)
        SD = bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,
                                   text='ارسل رقم الهاتف بالصيغه الدوليه', reply_markup=nc1)
...
═════════𝚃𝙸𝙺𝚃𝙾𝙺═══════════
'''
            bot.send_message(message.chat.id, f"<strong>{msg}</strong>", parse_mode="html",
                             reply_markup=types.InlineKeyboardMarkup())
            bot.send_message(message.chat.id, f"<strong>اضغط [ /start ] للرجوع الى القائمه</strong>", parse_mode="html",
                             reply_markup=types.InlineKeyboardMarkup())

        except (KeyError, IndexError):
            msger = f'''Error Username 🚫 ⇾ {fm}\nTry again'''
            bot.send_message(message.chat.id, f"<strong>{msger}</strong>", parse_mode="html",
                             reply_markup=types.InlineKeyboardMarkup())
            bot.send_message(message.chat.id, f"<strong>اضغط [ /start ] للرجوع الى القائمه</strong>", parse_mode="html",
                             reply_markup=types.InlineKeyboardMarkup())

    except Exception as e:
        bot.send_message(message.chat.id, f"<strong> ❗لقد حدث خطأ ماا</strong>", parse_mode="html",
                         reply_markup=types.InlineKeyboardMarkup())
        bot.send_message(message.chat.id, f"<strong>اضغط [ /start ] للرجوع الى القائمه</strong>", parse_mode="html",
                         reply_markup=types.InlineKeyboardMarkup())
...
═════════𝚃𝚠𝚒𝚝𝚝𝚎𝚛 𝚇═══════════
        ...
        bot.send_message(message.chat.id, f"<strong>❗ خطأ في اليوزر</strong>", parse_mode="html",
                         reply_markup=types.InlineKeyboardMarkup())
        bot.send_message(message.chat.id, f"<strong>اضغط [ /start ] للرجوع الى القائمه</strong>", parse_mode="html",
                         reply_markup=types.InlineKeyboardMarkup())
...
def VI(message):
    ...
    try:
        ...
        try:
            sub = str(r.split('"subscriberCountText":{"accessibility":{"accessibilityData":{"label":"')[1]).split('"')[
                0]
        except:
            sub = ""
        try:
...
═════════𝚃𝚎𝚕𝚎𝚐𝚛𝚊𝚖═══════════
        ...
        bot.send_message(message.chat.id, f"<strong> خطا في اليوزر</strong>", parse_mode="html",
                         reply_markup=types.InlineKeyboardMarkup())
        bot.send_message(message.chat.id, f"<strong>اضغط [ /start ] للرجوع الى القائمه</strong>", parse_mode="html",
                         reply_markup=types.InlineKeyboardMarkup())
...
═════════𝙸𝚗𝚏𝚘𝙿𝚑𝚘𝚗𝚎═══════════
        ...
        bot.send_message(message.chat.id, f"<strong> ❗لقد حدث خطأ  ارسل الرقم مع رمز الدوله + </strong>",
